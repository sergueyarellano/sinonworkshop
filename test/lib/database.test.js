const test = require('tape'),
    sinon = require('sinon'),
    databaseConnection = require('../../lib/database.js')
...
test('get Full name from database', (assert) => {
    const response = {
        ...
        stub = sinon.stub(databaseConnection, 'getUserInfo').returns(response)
...
test('get Error FOR INVALID USER', (assert) => {
    const mock = sinon.mock(databaseConnection),
        spy = sinon.spy(databaseConnection, 'getManOfTheDay')
